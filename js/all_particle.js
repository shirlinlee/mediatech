...
var container,
    ...
    signUpFirst = true,
    $signUp =$('.signUp'),
    signUp_top = $signUp.offset().top,
    kv_text = $('.index').find('h2,p'),
    kv_text_wrap = $('.index').find('.wording'),
    scroll_val = 0,
    scrolling = false,
...
function onWindowFocus(e) {
  ...
  $(window).scrollTop( scroll_val+0.1 );
}
...
function onWindowScroll(e) {
  ...
  if( scroll_val >= intro_top + 50){
    ...
    if( scroll_val > half && scroll_val <= window.innerHeight) {
      kv_text.css({opacity:half/scroll_val });
      kv_text_wrap.css({marginTop: (half- scroll_val)/2});
    } else {
      kv_text.css({opacity:1 });
      kv_text_wrap.css({marginTop: 0});
    }
  ...
  if (firstLoad ) {
    // 做介紹動態
    if( scroll_val >= intro_top/3 && scroll_val < intro_bottom) {
      if(intro_people) {
        $('.intro_text').addClass('animated');
        intro_people = false;
      }
      if( intro_text && scroll_val >= intro_top/1.5 ) {
        $('.general,.curate').addClass('animated');
        $('.introduce .photo').addClass('animated');
        intro_text = false;
      }
    } 
    // 做講師動態
    if( lecturerFirst && scroll_val >= ( agenda_end - bodyHeight/2) && scroll_val < ( signUp_top -  bodyHeight/2 ) ) {
        $('.lecturer .main').addClass('animated');
    ...
    // 做報名動態
    if( signUpFirst && scroll_val >= ( signUp_top -  bodyHeight/2 )) {
        $signUp.find('.main').addClass('animated');
        signUpFirst = false;
    }

    //全部做完關掉
    if(!signUpFirst && !lecturerFirst && !intro_text && !intro_people ){
      firstLoad = false;
    }
